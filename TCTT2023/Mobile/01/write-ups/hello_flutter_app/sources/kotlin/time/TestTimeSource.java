...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1485plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1397toLongimpl(duration, getUnit());
        if (longDelta != Long.MIN_VALUE && longDelta != LongCompanionObject.MAX_VALUE) {
            long j = this.reading;
            newReading = j + longDelta;
            if ((j ^ longDelta) >= 0 && (j ^ newReading) < 0) {
                m1484overflowLRDsOJo(duration);
            }
        } else {
            double delta = Duration.m1394toDoubleimpl(duration, getUnit());
    ...
    private final void m1484overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + "ns is advanced by " + ((Object) Duration.m1400toStringimpl(duration)) + '.');
    }
}
